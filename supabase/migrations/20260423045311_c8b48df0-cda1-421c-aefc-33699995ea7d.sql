...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.user_roles (user_id, role) VALUES (v_uid, v_role::app_role) ON CONFLICT DO NOTHING;

    INSERT INTO public.profiles (id, email, full_name, phone, mobile_verified)
    VALUES (v_uid, rec->>'email', rec->>'full_name', rec->>'phone', true)
    ON CONFLICT (id) DO UPDATE SET full_name = EXCLUDED.full_name, phone = EXCLUDED.phone, mobile_verified = true;

    IF v_role = 'worker' THEN
      -- Map junior/mid/senior -> beginner/intermediate/advanced for worker_skills constraint
      v_proficiency := CASE rec->>'skill_level'
      ...
      INSERT INTO public.worker_profiles (
        ...
        languages, ecr_status, ecr_category, open_to_relocation, preferred_shift, onboarding_completed
      ) VALUES (
        v_uid, rec->>'bio', 'Indian', rec->>'city',
        (rec->>'city') || ', ' || (rec->>'state'), 'India',
        rec->>'role_title', (rec->>'experience')::int, rec->>'skill_level',
        (rec->>'salary_min')::numeric, (rec->>'salary_max')::numeric, 'INR',
        rec->>'availability', (rec->>'has_passport')::boolean, (rec->>'has_visa')::boolean,
        ARRAY[rec->>'country_pref']::text[], ARRAY['Hindi','English']::text[],
        CASE WHEN (rec->>'has_passport')::boolean THEN 'required' ELSE 'not_checked' END,
        CASE WHEN (rec->>'has_passport')::boolean THEN 'ECR' ELSE NULL END,
        true, 'Day', true
        ...
        skill_level = EXCLUDED.skill_level, expected_salary_min = EXCLUDED.expected_salary_min,
        expected_salary_max = EXCLUDED.expected_salary_max, currency = 'INR',
        availability = EXCLUDED.availability, has_passport = EXCLUDED.has_passport, has_visa = EXCLUDED.has_visa,
        visa_countries = EXCLUDED.visa_countries, languages = EXCLUDED.languages,
        ecr_status = EXCLUDED.ecr_status, ecr_category = EXCLUDED.ecr_category,
        open_to_relocation = true, onboarding_completed = true;

      INSERT INTO public.worker_skills (worker_id, skill_name, proficiency_level, years_of_experience)
      VALUES (v_uid, rec->>'role_title', v_proficiency, (rec->>'experience')::int)
    ...
    ELSIF v_role = 'employer' THEN
      INSERT INTO public.employer_profiles (
        ...
        v_uid, rec->>'company_name', rec->>'industry', rec->>'business_type', rec->>'company_size',
        'India', rec->>'state', (rec->>'city') || ', ' || (rec->>'state'),
        rec->>'website', rec->>'bio',
        ...
        business_type = EXCLUDED.business_type, company_size = EXCLUDED.company_size,
        country = 'India', office_state = EXCLUDED.office_state, office_address = EXCLUDED.office_address,
        website = EXCLUDED.website, bio = EXCLUDED.bio,
        preferred_countries = EXCLUDED.preferred_countries, workers_required = EXCLUDED.workers_required,
        hiring_roles = EXCLUDED.hiring_roles, onboarding_completed = true;
    END IF;
  END LOOP;
  RETURN v_count;
END;
